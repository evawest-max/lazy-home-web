...
export default function ListPropertyStep4({ formData, setFormData, onBack, onSubmit }) {
    ...
    const requiredFields = ['accountName', 'bankName', 'accountNumber'];
    ...
    useEffect(() => {
        if (formData.accountNumber.length === 10 && formData.bankName && formData.accountName) {
            setTimeout(() => setAccountVerified(true), 500);
        } else {
            setAccountVerified(false);
        }
    }, [formData.accountNumber, formData.bankName, formData.accountName]);

    const canSubmit = formData.accountName && formData.bankName && formData.accountNumber.length === 10 &&
        formData.termsAccepted && formData.escrowAccepted && formData.policyAccepted;
            ...
            <VStack align="stretch" px={6} mt={-4} spacing={6}>
                ...
                <Box bg="white" borderRadius="xl" p={6} boxShadow="sm">
                    <VStack align="stretch" spacing={5}>
                        ...
                        <FormControl isRequired isInvalid={formError && !formData.accountName}>
                        ...
                        <FormControl isRequired isInvalid={formError && !formData.bankName}>
                            <FormLabel color="brand.gray.700" fontSize="sm" fontWeight="600">
                                Bank Name
                            </FormLabel>
                            <Select
                        ...
                        <FormControl isRequired isInvalid={formError && (!formData.accountNumber || formData.accountNumber.length !== 10)}>
                            <FormLabel color="brand.gray.700" fontSize="sm" fontWeight="600">
                                Account Number
                            </FormLabel>
                            <Input
