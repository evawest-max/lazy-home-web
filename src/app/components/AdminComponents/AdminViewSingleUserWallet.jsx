...
import { freezeWallet, getWalletDetailsByWalletId, unfreezeWallet } from '../../../../api';
...
export default function AdminViewSingleUserWallet() {
  ...
  useEffect(() => {
    const fetchWalletDetails = async () => {
      if (!walletId) {
        setError('No wallet was selected.');
        setLoading(false);
        return;
      }
      ...
      try {
        const res = await getWalletDetailsByWalletId(walletId);
        ...
        setWalletData({
          ...
          txPagination: payload?.txPagination ?? { total: transactions.length, page: 1, limit: transactions.length || 10, pages: 1 },
        });
      } catch (err) {
    ...
    return [
      {
        ...
        value: formatCurrency(walletData.wallet?.balance ?? 0),
              ...
              <Card shadow="md">
                ...
                <CardBody>
                  <VStack align="stretch" spacing={3}>
                    ...
                    <Flex justify="space-between"><Text color="gray.500">Total Credits</Text><Text fontWeight="semibold">{formatCurrency(walletData.txStats?.totalCredits ?? 0)}</Text></Flex>
                    <Flex justify="space-between"><Text color="gray.500">Total Debits</Text><Text fontWeight="semibold">{formatCurrency(walletData.txStats?.totalDebits ?? 0)}</Text></Flex>
